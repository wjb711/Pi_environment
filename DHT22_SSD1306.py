#树莓派+DHT22温湿度探头+SSD1306 小屏oled
import Adafruit_DHT
from display import display_led as display
import datetime
import json
import subprocess
import time
...
#外部邮件的发送方法
    ...
    #receivers = ['xxxx@163.com','10054053@qq.com']  # 接收邮件，可设置为你的QQ邮箱或者其他邮箱
...
#内部邮件的发送方法
    ...
    msg['Subject'] = title
    smtp.sendmail('hello@compay.com', receivers, msg.as_string())
    smtp.quit()
...
#主函数
    ...
    hum_MAX=90
    #设定温度湿度最大值
    receivers=['xxxx@company.com','xxxx1@company.com']
    #报警接收人
    ...
    #如果湿度超过99， 传感器有些问题，经常湿度会到3000+，引起不必要的麻烦，故这里使用软件方式跳过这些非正常值
        ...
        display("Temperature:"+tmp+"C","Humidity:"+hmt+"%",2,3)
        #led显示器上显示温度，湿度
        writejson()
